...
Loads weights from a habitat-baselines checkpoint via key remapping.
Compatible with PyTorch ≥ 2.6 (explicit weights_only=False).
"""
...
class PointNavRGBDModel(nn.Module):
    ...
    Loads weights from a habitat-baselines checkpoint (.pth) via key
    remapping, so it works without any habitat / habitat-baselines
    dependency.
    """
    ...
    def _load_weights(self, path):
        ...
        for ck, cv in state.items():
            ...
            nk = nk.replace(
                "net.visual_encoder.running_mean_and_var.",
                "visual_encoder.running_mean_and_var.",
            )
            nk = nk.replace(
                "net.visual_encoder.compression.", "visual_encoder.compression."
            )
    ...
    def forward(self, rgb, depth, goal_vector, prev_action=0):
        """
        Args:
            rgb:   (1, 3, 256, 256) float32 [0, 1]   — CHW format
            depth: (1, 1, 256, 256) float32           — CHW format
